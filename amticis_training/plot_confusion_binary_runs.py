"""Plot side-by-side binary confusion matrices for the three natively-trained
binary CVFSNet runs (coronal, sagittal, fusion) with accuracy / F1 / AUROC /
AUPRC annotated per panel.

Unlike ``mae_ordinal/plot_confusion_binary.py`` (which collapses fuse01
4-class predictions to binary post-hoc), these runs were trained directly
with ``data.label_mode: binary`` (model_num_class=2), so the ``fuse`` head's
softmax output is used as-is.

    uv run python -m amticis_training.plot_confusion_binary_runs

Outputs:
    plots/confusion_matrices_binary_runs_cvfsnet.png
    Assets/confusion_matrix_metrics_cvfsnet_binary_runs.csv
"""
...
RUNS = [
    ("Coronal", "cvfsnet_pt_binary_cor_v3"),
    ("Sagittal", "cvfsnet_pt_binary_sag_v3"),
    ("Fusion", "cvfsnet_pt_binary_fusion_v3"),
]
...
@torch.no_grad()
def _predict(run_name: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for batch in datamodule.val_dataloader():
        ...
        fuse_logits = outputs[0]
        probs = torch.softmax(fuse_logits, dim=-1).cpu().numpy()
...
def main() -> None:
    ...
    plot_path = REPO_ROOT / "plots" / "confusion_matrices_binary_runs_cvfsnet.png"
    csv_rows: List[dict] = []

    for ax, (title, run_name) in zip(axes, RUNS):
        preds, targets, scores = _predict(run_name)
        metrics = _compute_metrics(preds, targets, scores)
        _plot_panel(ax, f"CVFSNet {title} (n={metrics['n']})", preds, targets, metrics)
        print(
    ...
    fig.suptitle(
        ...
        "Natively-trained binary CVFSNet (label_mode=binary)",
        fontsize=13,
    )
    ...
    metrics_csv = REPO_ROOT / "Assets" / "confusion_matrix_metrics_cvfsnet_binary_runs.csv"
